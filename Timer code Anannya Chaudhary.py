'''The start button starts the countdown and also acts like resume button.'''
# Import the time module
import winsound      #This beeps the PC or laptop speaker. This accepts two arguments: frequency and duration
winsound.PlaySound('FILENAMEHERE.wav', winsound.SND_FILENAME)       # It accepts two arguments namely sound and flags. The sound parameter is useful to pass the audio data or the filename that contains the audio.
import time     
from tkinter import *   #Tkinter is the standard GUI library for Python. Python when combined with Tkinter provides a fast and easy way to create GUI applications. It is used to create a window
...
class CountDown:
    def __init__(self, root):
        ...
        font=('Helvetica',12), bg="white", fg="black", 
        command=self.Reset)
        reset_button.place(x=240, y=250)
    ...
    def Get_Time(self):
        ...
        self.initial_hour = self.hour_combobox.get()
        self.initial_minute = self.minute_combobox.get()
        self.initial_second = self.second_combobox.get()
    ...
    # This function will run when the reset button is pressed
    def Reset(self):
        self.pause = True
        self.time_display.config(text=f"Time Left: {self.initial_hour}: {self.initial_minute}: {self.initial_second}")
